...
void loop() {
  // payload "1234567890 <counter>"
  char buf[48];
  int n = snprintf(buf, sizeof(buf), "%s %u", message, counter);
  if (n < 0) n = 0;
  if (n > 255) n = 255;

  // explicit header – délka do hlavičky
  LoRa.setPayloadLength((uint8_t)n);

  LoRa.beginPacket();
  LoRa.write((uint8_t*)buf, (uint8_t)n);  // nebo LoRa.write((char*)buf, (uint8_t)n);
  LoRa.endPacket();

  LoRa.wait(); // na TX_DONE (přes IRQ/polling)
